...
│ │ └ ports/                       # Boundary contracts (try to separate into different files according to library / or even use case)
...
2.  **Inject**:
    ...
    doc.head.insertAdjacentHTML(
      "beforeend",
      `<style id="logseq-doc-agent-css">${cssContent}</style>`,
    );
    ```
...
> **Comprehensive E2E guide**: See [`docs/e2e_test_creation_guideline.md`](docs/e2e_test_creation_guideline.md) for the full template, locator strategies (main window vs plugin iframe), ID remapping, and troubleshooting.
