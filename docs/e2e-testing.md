...
For the `db` channel, the managed test graph is deployed into Logseq's built-in demo graph directory under `logseq-environments/e2e/db/home/logseq/graphs/Demo` rather than a sibling `graph/` directory.
