...
graph TD
    ...
    subgraph "Tier 2: Electron (Integration)"
        ElectronTest[tests/e2e/electron/*.spec.ts] --> LogseqBinary[.logseq/app/...]
...
The Logseq binary source is configured in `logseq-versions.json`. The setup script:
...
4. Extracts and caches it in `.logseq/app/...`
