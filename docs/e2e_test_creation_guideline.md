...
2.  **Snapshot**: Capture the state of the graph *at the moment* the AI was called (this provides the "source" for ID remapping).
...
The remapper needs to know what the graph looked like *at the time of recording*.
...
2.  Save this as a text file in `tests/graph-template/snapshots/<Page Name>.txt`.
    *Format must follow the outliner syntax used by the agent (e.g., `- id:123 Content`).*
...
test("Feature description", async ({}, testInfo) => {
  ...
  // 3. Built Content-Based ID Remap
  // This is the "magic" step that maps recorded IDs to the fresh graph's IDs
  const currentText = await getPageTreeFromLiveApp(window, "Page Name"); // Helper to extract tree
  ...
  await window.evaluate((data) => {
    // Inject into plugin iframe window object
    window.iframes.find(...).__LDA_REPLAY_CHATLOG__ = data;
  }, remappedChatlog);

  // 5. Trigger & Verify
  await window.click('.open-chat');
  await window.fill('textarea', 'User message'); // Trigger logic
