...
export abstract class BaseBlockInjector<TData> {
  ...
  protected fetchBlockContent(): boolean {
    return false;
  }
  ...
  public async execute() {
    try {
      ...
      console.log(
        `[${this.logPrefix}] Current page: ${currentPage.originalName || currentPage.name} (${currentPage.uuid})`,
      );
        ...
        try {
          ...
          if (config.targetSelector) {
            ...
            if (targetContainer) {
              ...
              console.warn(
                `[${this.logPrefix}] Skipping ${blockId}: Target selector ${config.targetSelector} not found.`,
              );
              continue;
            }
          }
          ...
          if (this.checkForExistingInjection(targetElement)) {
            console.log(
              `[${this.logPrefix}] Skipping ${blockId}: Injected component already present`,
            );
            continue;
          }
          ...
          const rawContent = await this.logseqApi.Editor.getBlockPropertyContent(
            blockId,
            propertyName,
          );
          ...
          if (rawContent) {
            ...
            console.warn(
              `[${this.logPrefix}] Property ${propertyName} not found or empty for ${blockId}.`,
            );
