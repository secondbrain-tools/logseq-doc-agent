...
export class EvaluationReviewService {
    ...
    private async mutateEvaluation(blockUuid: string, mutator: (evaluation: BlockEvaluation) => void): Promise<void> {
        ...
        const evalString = block.properties?.[LDA_EVALUATION_PROPERTY.replace('logseq-doc-agent.', '')]
            || block.properties?.[LDA_EVALUATION_PROPERTY]
            || block.properties?.[LDA_EVALUATION_PROPERTY_CAMEL];

        if (!evalString) throw new Error(`Block ${blockUuid} has no evaluation property. Looked for: ${LDA_EVALUATION_PROPERTY_CAMEL}`);
        ...
        await this.logseq.upsertBlockProperty(blockUuid, LDA_EVALUATION_PROPERTY, JSON.stringify(evaluation));
    }
}
