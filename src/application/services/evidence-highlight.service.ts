...
export class EvidenceHighlightService {
  ...
  public async focusIssue(blockId: string, issue: Issue): Promise<void> {
    ...
    for (const rawId of rawIds) {
      if (gen !== this.focusGeneration) {
        console.log(
          `${LOG_PREFIX} focusIssue() gen=${gen} cancelled (now gen=${this.focusGeneration})`,
        );
        return;
      }
    ...
    if (gen !== this.focusGeneration) {
      console.log(
        `${LOG_PREFIX} focusIssue() gen=${gen} cancelled after resolution (now gen=${this.focusGeneration})`,
      );
      return;
    }
    ...
    const issueResolvedSourceId = issueRawSourceId
      ? (resolvedMap.get(issueRawSourceId) ?? null)
      : null;

    console.log(
      `${LOG_PREFIX} gen=${gen} issueRawSourceId="${issueRawSourceId}" => resolved="${issueResolvedSourceId}"`,
    );
   ...
   */
  public async previewSuggestion(
    blockId: string,
    suggestion: Suggestion,
    rawSourceId?: string | null,
  ): Promise<void> {
    console.log(
      `${LOG_PREFIX} previewSuggestion() blockId="${blockId}" rawSourceId="${rawSourceId}" exact="${suggestion.selector?.exact}"`,
    );
