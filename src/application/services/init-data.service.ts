...
export class InitDataService {
  ...
  async initialize() {
    ...
    if (promptsResult.isNew) {
      await this.logseqApi.appendBlockInPage(
        `${storageRoot}/prompts`,
        "Storage for user-defined and built-in prompts.",
      );
    }
    ...
    if (agentsResult.isNew) {
      await this.logseqApi.appendBlockInPage(
        `${storageRoot}/agents`,
        "Storage for built-in and user-defined agent definitions.",
      );
    }
  ...
  private async ensurePage(name: string): Promise<{ page: any; isNew: boolean }> {
    ...
    if (!page) {
      ...
      page = await this.logseqApi.createPage(
        name,
        {},
  ...
  private async syncRootPage(pageName: string) {
    ...
    if (existingBlocks.length > 0) {
      ...
      const versionProp =
        parsedProps["logseq-doc-agent.root-version"] || parsedProps["logseqDocAgent.rootVersion"];
    ...
    if (ROOT_PAGE_CONFIG.version > currentVersion) {
      console.log(
        `[InitDataService] Updating root page content to version ${ROOT_PAGE_CONFIG.version} (was ${currentVersion})`,
      );
  ...
  private async syncBuiltinPrompts(pageName: string) {
    ...
    for (const prompt of BUILTIN_PROMPTS) {
      ...
      if (existingBlock && existingBlock.uuid) {
        ...
        const propVal =
          parsedProps[BUILTIN_MARKER_PROPERTY] ||
          parsedProps[BUILTIN_MARKER_PROPERTY.toLowerCase()];
        ...
        if (propVal !== undefined) {
          ...
          const match = existingBlock.content.match(
            new RegExp(`${BUILTIN_MARKER_PROPERTY}::\\s*(\\d+)`),
          );
          if (match && match[1]) {
        ...
        if (prompt.version > existingVersion) {
          ...
          console.log(
            `[InitDataService] Updating built-in prompt '${prompt.name}' to version ${prompt.version} (was ${existingVersion})`,
          );
          ...
          console.log(
            `[InitDataService] Built-in prompt '${prompt.name}' is up to date (version ${existingVersion})`,
          );
        }
      } else {
        // Create parent block
    ...
    for (const child of children) {
      try {
        const childBlock = await this.logseqApi.insertBlock(
          lastBlockUuid,
          child.content,
          isFirst ? { sibling: false } : { sibling: true, before: false },
        );
  ...
  private async ensureNoticeBlock(pageName: string, existingBlocks: any[]) {
    ...
    if (existingNotice && existingNotice.uuid) {
      ...
      await this.logseqApi.insertBlock(existingBlocks[0].uuid, noticeContent, {
        before: false,
        sibling: true,
      });
    } else {
  ...
  private findBlockByPromptName(blocks: any[], promptName: string): any | null {
    for (const block of blocks) {
      ...
      if (block.properties) {
        const nameProp =
          block.properties[LDA_PROMPT_NAME_PROPERTY] ||
  ...
  async migrateStorageRoot(oldRoot: string, newRoot: string) {
    ...
    if (oldPage) {
      await this.logseqApi.UI.showMsg(
        `Renaming storage page from '${oldRoot}' to '${newRoot}'`,
        "info",
      );
      await this.logseqApi.renamePage(oldRoot, newRoot);
    } else {
      console.log(
        `[InitDataService] Old storage page '${oldRoot}' not found. Initializing new structure.`,
      );
