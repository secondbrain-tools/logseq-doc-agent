...
import {
  ...
  type ContextScope,
  type IssueReplyResponse,
} from "../../domain/evaluation/issue-reply.types";
...
export class IssueReplyService {
  ...
  async replyToIssue(
    ...
    const conversationHistory = (issue.user_feedback || []).map((fb) => {
      ...
      else if (fb.type === "self_suggestion" || fb.type === "self_assessment")
        prefix = "[USER PRE-COMMITMENT DATA]: ";
...
${evaluationContext}
    ...
    await this.evaluationReviewService.mutateEvaluation(blockUuid, (ev) => {
      ...
      if (replyObject.updated_description)
        targetIssue.description = replyObject.updated_description;
      if (replyObject.counterargument !== undefined)
        targetIssue.counterargument = replyObject.counterargument || undefined;
    ...
    const evalString =
      updatedBlock?.properties?.["logseq-doc-agent.evaluation"] ||
      updatedBlock?.properties?.["evaluation"] ||
      updatedBlock?.properties?.["logseqDocAgent.evaluation"];
