...
export class PromptTemplateService {
  ...
  async resolvePrompt(promptName: string): Promise<ChatPrompt | null> {
    ...
    const inside = matchingPrompts.filter((p) =>
      p.pageName.toLowerCase().startsWith(namespacePrefix),
    );
    const outside = matchingPrompts.filter(
      (p) => !p.pageName.toLowerCase().startsWith(namespacePrefix),
    );

    if (outside.length > 0) {
      if (outside.length > 1) {
        this.warnUser(
          `Multiple prompts found with name "${promptName}" outside the namespace. Using the first one from page "${outside[0].pageName}".`,
        );
      }
      return outside[0];
    }
    ...
    if (inside.length > 1) {
      this.warnUser(
        `Multiple prompts found with name "${promptName}" inside the namespace. Using the first one from page "${inside[0].pageName}".`,
      );
    }
    return inside[0];
  }
