...
On partial failure, returns tree built so far + error.`,
    inputSchema: z.object({
        ...
        content: z.string().describe('Block content. Supports nested blocks using "- " (unordered) or "N. " (ordered) list syntax with indentation.'),
        anchor: z.enum(['parent', 'before', 'after']).optional().describe('Where to insert relative to target. Default is "parent" (append as child).'),
        parse_subtrees: z.boolean().optional().describe('If true (default), parse "- " and "N. " lists as nested child blocks. If false, escape lists to preserve as literal text.'),
        ordered: z.boolean().optional().describe('If true, mark the root inserted block as an ordered (numbered) list item by setting logseq.order-list-type:: number. Does not affect children parsed from subtree syntax.'),
    }),
    execute: async ({
        targetId,
        content,
