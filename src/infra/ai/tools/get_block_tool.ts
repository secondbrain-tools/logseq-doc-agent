...
import { applyMergeLogicToTree, buildDocumentResponse } from "./get_logseq_document_tool";
import type { LogseqSelection } from "./types";
import { sanitizeBlockId } from "./tool-utils";
import { getCurrentLogseqApi } from "../../logseq";

export const createGetBlockTool = (context: { mergeDefault: boolean; mergeBoth: boolean }) =>
  tool({
    description:
      "Returns the content of a specific block given its UUID. Can optionally return the entire subtree of children blocks.",
    inputSchema: z.object({
      blockId: z
        .union([z.string(), z.number()])
        .describe("The UUID or integer ID of the block to retrieve"),
      subtree: z
        .boolean()
        .optional()
        .default(true)
        .describe("Whether to include children blocks (subtree). Defaults to true."),
    }),
    execute: async ({
      blockId,
      subtree = true,
