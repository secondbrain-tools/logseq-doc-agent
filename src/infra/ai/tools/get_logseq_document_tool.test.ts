...
describe("getLogseqDocument tool", () => {
  ...
  describe("formatBlockTree", () => {
      ...
      const blocks = [
        {
          id: 1,
          content: "Parent",
          children: [{ id: 2, content: "Child", children: [] }],
        },
      ];
      const output = formatBlockTree(blocks as any);
    ...
    it("should handle multi-line content", () => {
      const blocks = [{ id: 1, content: "Line 1\nLine 2", children: [] }];
      const output = formatBlockTree(blocks as any);
    ...
    it("should handle blocks without numeric ID (use UUID)", () => {
      const blocks = [{ uuid: "abc-123", content: "No ID Block", children: [] }];
      const output = formatBlockTree(blocks as any);
