...
describe('submitBlockEvaluation tool', () => {
    ...
    it('should remove all known evaluation key variants before upserting to guarantee overwrite', async () => {
        ...
        const firstEvaluation = {
            results: [{ criterion_id: 'clarity', category: 'Writing', score: 3, reason: 'OK', confidence: 60, issues: [] }],
            summary: { overall_score: 3, overall_reason: 'Average', category_aggregates: [] }
        };
        const secondEvaluation = {
            results: [{ criterion_id: 'clarity', category: 'Writing', score: 5, reason: 'Excellent', confidence: 95, issues: [] }],
            summary: { overall_score: 5, overall_reason: 'Excellent', category_aggregates: [] }
        };

        await tool.execute({ block_id: 1, evaluation: firstEvaluation }, { toolCallId: 'test-1', messages: [] });
        await tool.execute({ block_id: 1, evaluation: secondEvaluation }, { toolCallId: 'test-2', messages: [] });

        // Each call should remove all three known key variants
        expect(mockLogseq.Editor.removeBlockProperty).toHaveBeenCalledWith('uuid-1', LDA_EVALUATION_PROPERTY);
        expect(mockLogseq.Editor.removeBlockProperty).toHaveBeenCalledWith('uuid-1', LDA_EVALUATION_PROPERTY_CAMEL);
