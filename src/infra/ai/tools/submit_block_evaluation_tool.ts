...
export function normalizeCategories(evaluation: any): void {
  ...
  if (allSingle) {
    ...
    evaluation.results.forEach((r: any) => {
      r.category = null;
    });
  } else {
...
export const createSubmitBlockEvaluationTool = () => {
  ...
  try {
    const suggestionAlternativesStr =
      ((window as any).logseq?.settings?.["cognitiveForcing_suggestionAlternatives"] as string) ||
      "1";
  ...
  return tool({
    ...
    inputSchema: z.object({
      block_id: z
        ...
        .describe("Identifier of the block being evaluated (provided by the host application)."),
      evaluation: schema,
    }),
    execute: async ({ block_id, evaluation }: { block_id: number | string; evaluation: any }) => {
      try {
