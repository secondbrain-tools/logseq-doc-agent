...
export class VercelAIAdapter implements IAIService {
  ...
  async streamAgent(
    ...
    if (win.__LDA_REPLAY_CHATLOG__) {
      console.log(
        "[VercelAIAdapter] Replay chatlog detected — delegating to ChatlogReplayAIService",
      );
      ...
      return replayService.streamAgent(
        messages,
        modelId,
        providerId,
        merge,
    ...
    const { model, options } = this.modelFactory.configureModel(
      modelId,
      providerId,
      reasoningEffort,
    );
    ...
    if (agentContext?.prompt) {
      ...
      const suggestionAlternativesStr = this.settingsAdapter.get<string>(
        "cognitiveForcing_suggestionAlternatives",
        "1",
      );
      const suggestionAlternatives = parseInt(suggestionAlternativesStr, 10) || 1;
      const preCommitmentPrompt = this.settingsAdapter.get<boolean>(
        "cognitiveForcing_preCommitmentPrompt",
        false,
      );
      const counterargument = this.settingsAdapter.get<boolean>(
        "cognitiveForcing_counterargument",
        false,
      );
      ...
      coreMessages = [{ role: "system", content: systemPrompt }, ...coreMessages];
