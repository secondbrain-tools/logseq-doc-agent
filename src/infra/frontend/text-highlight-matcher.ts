...
export function findBestMatch(text: string, selector: TextQuoteSelector): MatchResult | null {
  ...
  while (searchIdx < haystack.length) {
    ...
    if (suffix) {
      ...
      if (availableSuffixLen > 0) {
        const actualSuffix = haystack.substring(
          haystackEndIdx,
          haystackEndIdx + availableSuffixLen,
        );
  ...
  while (
    selectorStartIdx < selector.exact.length &&
    !/^[a-z0-9]$/i.test(
      selector.exact[selectorStartIdx].normalize("NFD").replace(/[\u0300-\u036f]/g, ""),
    )
  ) {
    selectorStartIdx++;
  }
  ...
  while (
    selectorEndIdx >= 0 &&
    !/^[a-z0-9]$/i.test(
      selector.exact[selectorEndIdx].normalize("NFD").replace(/[\u0300-\u036f]/g, ""),
    )
  ) {
    selectorEndIdx--;
  }
