...
export class TextHighlighter implements HighlightPort {
  ...
  private resolveTargetBlockId(fallbackBlockId: string, sourceId?: string | null): string {
    ...
    console.log(
      `${LOG_PREFIX} resolveTargetBlockId(fallback="${fallbackBlockId}", sourceId=${sourceId}) => "${resolved}"`,
    );
    return resolved;
  }
  ...
  private getTextContainers(blockEl: HTMLElement): HTMLElement[] {
    ...
    console.log(
      `${LOG_PREFIX} getTextContainers => found ${containers.length} container(s): [${containers.map((c) => c.className).join(", ")}]`,
    );
    return containers;
  }
  ...
  ): { textNodes: TextNodeMapping[]; plainText: string } {
  ...
  public highlight(
    ...
    console.log(
      `${LOG_PREFIX} highlight() called with parentBlockId="${parentBlockId}", ${selectors.length} selector(s)`,
    );
    ...
    for (const { selector, source_id } of selectors) {
      console.log(
        `${LOG_PREFIX} Processing selector: exact="${selector.exact}", prefix="${selector.prefix}", suffix="${selector.suffix}", source_id=${source_id}`,
      );
      ...
      if (containers.length === 0) {
        console.warn(
          `${LOG_PREFIX} No text containers found in block "${targetBlockId}", skipping.`,
        );
        continue;
      }
      ...
      if (!matched) {
        ...
        console.log(
          `${LOG_PREFIX} No match in individual containers, trying combined text search...`,
        );
        const combinedHighlights = this.highlightSelectorAcrossContainers(
          doc,
          containers,
          selector,
        );
      ...
      if (!matched) {
        ...
        if (descendantHighlights.length > 0) {
          ...
          console.warn(
            `${LOG_PREFIX} No match found for selector exact="${selector.exact}" in block "${targetBlockId}" or any of its descendants`,
          );
        }
      }
    }
    ...
    console.log(
      `${LOG_PREFIX} highlight() complete. ${this.activeHighlights.length} highlight(s) applied.`,
    );
  }
   ...
   */
  private highlightSelectorInDescendants(
    doc: Document,
    blockEl: HTMLElement,
    ...
    console.log(
      `${LOG_PREFIX} Searching ${descendants.length} descendant block(s) for exact="${selector.exact.substring(0, 60)}"`,
    );
    ...
    for (const descendant of descendants) {
      ...
      for (const container of containers) {
        ...
        if (highlights.length > 0) {
          console.log(
            `${LOG_PREFIX} Match found in descendant block "${descendant.getAttribute("blockid")}"`,
          );
          return highlights;
        }
      }
      ...
      if (combined.length > 0) {
        console.log(
          `${LOG_PREFIX} Match found (combined) in descendant block "${descendant.getAttribute("blockid")}"`,
        );
        return combined;
      }
    }
   ...
   */
  private findMatchInDescendants(
    doc: Document,
    blockEl: HTMLElement,
    selector: TextQuoteSelector,
  ): MatchInTextNodes | null {
    const descendants = this.getDescendantBlockElements(blockEl);
    console.log(
      `${LOG_PREFIX} Searching ${descendants.length} descendant block(s) for preview match exact="${selector.exact.substring(0, 60)}"`,
    );
    ...
    for (const descendant of descendants) {
      ...
      if (match) {
        console.log(
          `${LOG_PREFIX} Preview match found in descendant block "${descendant.getAttribute("blockid")}"`,
        );
        return match;
      }
    }
   ...
   */
  private highlightSelectorAcrossContainers(
    doc: Document,
    ...
    console.log(
      `${LOG_PREFIX} Combined text (${plainText.length} chars): "${plainText.substring(0, 200)}${plainText.length > 200 ? "..." : ""}"`,
    );
    ...
    console.log(
      `${LOG_PREFIX} Combined match found at [${match.startOffset}, ${match.endOffset}]: "${plainText.substring(match.startOffset, match.endOffset)}"`,
    );
   ...
   */
  private highlightSelectorInContainer(
    doc: Document,
    ...
    console.log(
      `${LOG_PREFIX} Container "${container.className}" text (${plainText.length} chars): "${plainText.substring(0, 200)}${plainText.length > 200 ? "..." : ""}"`,
    );

    const match = findBestMatch(plainText, selector);
    if (!match) {
      console.log(
        `${LOG_PREFIX} findBestMatch returned null for container "${container.className}"`,
      );
      return [];
    }

    console.log(
      `${LOG_PREFIX} Match found at [${match.startOffset}, ${match.endOffset}] in "${container.className}": "${plainText.substring(match.startOffset, match.endOffset)}"`,
    );
    return this.applyHighlightRanges(doc, textNodes, match.startOffset, match.endOffset);
  }
  ...
  public clearHighlights(): void {
    if (this.activeHighlights.length > 0) {
      console.log(
        `${LOG_PREFIX} clearHighlights() removing ${this.activeHighlights.length} highlight(s)`,
      );
    }
  ...
  private findMatchInContainers(
    ...
    const { textNodes: combinedTextNodes, plainText: combinedPlainText } = this.collectTextNodes(
      doc,
      containers,
      true,
    );
  ...
  private buildReplacementPreviewWrapper(
    doc: Document,
    originalText: string,
    proposedText: string,
  ): HTMLElement {
  ...
  private applyReplacementPreview(
    doc: Document,
    rangesToHighlight: HighlightRange[],
    proposedText: string,
  ): HTMLElement | null {
    const originalText = rangesToHighlight
      .map((r) => r.node.textContent?.substring(r.startOffset, r.endOffset) || "")
      .join("");
  ...
  public previewSuggestion(blockId: string, suggestion: Suggestion): void {
    ...
    if (!matchData) {
      console.warn(
        `${LOG_PREFIX} Could not find text match for suggestion preview in block or descendants.`,
      );
      return;
    }
