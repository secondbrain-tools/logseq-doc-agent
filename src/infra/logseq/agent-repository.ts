...
export class LogseqAgentRepository implements IAgentRepository {
  ...
  async getAllAgents(): Promise<AgentDefinition[]> {
    try {
      ...
      const rawAgents = await Promise.all(blocks.map((block) => this.parseAgentFromBlock(block)));

      // Filter out null results and resolve duplicate names
      const agents = this.resolveDuplicateNames(
        rawAgents.filter((a): a is AgentDefinition => a !== null),
      );
  ...
  async ensureBuiltInAgentsExist(): Promise<boolean> {
    ...
    if (!agentsPage) {
      ...
      agentsPage = await this.logseqApi.createPage(
        agentsPageName,
        {},
      ...
      try {
        ...
        const findBlock = (blocks: any[]): any => {
          for (const b of blocks) {
            ...
            if (
              b.properties &&
              b.properties[LogseqAgentRepository.AGENT_PROPERTY] === agentConfig.name
            ) {
              return b;
            }
            if (
              b.properties &&
              b.properties[LogseqAgentRepository.AGENT_PROPERTY_CAMEL] === agentConfig.name
            ) {
              return b;
            }
            // Recursive
        ...
        if (existingBlock) {
          ...
          console.log(
            `[LogseqAgentRepository] Updating agent block ${agentConfig.name} from v${existingVersion} to v${agentConfig.version}`,
          );
...
${agentConfig.isDefault ? `${LogseqAgentRepository.DEFAULT_PROPERTY}:: true` : ""}`;
        ...
        console.error(
          `[LogseqAgentRepository] Error creating/updating agent block ${agentConfig.name}:`,
          error,
        );
      }
    }
  ...
  async ensureToolListBlock(): Promise<boolean> {
    ...
    if (existingBlock) {
      ...
      console.log(
        `[LogseqAgentRepository] Updating tool list from v${existingVersion} to v${TOOL_LIST_VERSION}`,
      );
  ...
  private async replaceBlockSubtree(blockUuid: string, version: number): Promise<void> {
    ...
    if (block?.children?.length) {
      for (const child of [...block.children].reverse()) {
        const childUuid =
          (child as any).uuid || (Array.isArray(child) ? child[1] : (child as any).id);
