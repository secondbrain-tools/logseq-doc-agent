...
describe("LogseqChatlogRepository - JSON File Storage", () => {
  ...
  beforeEach(() => {
    ...
    (mockApi.appendBlockInPage as any).mockImplementation(
      async (_page: string, content: string) => {
  ...
  describe("saveChatlog", () => {
    it("should save messages to JSON file", async () => {
      ...
      expect(mockStorage.setItem).toHaveBeenCalledWith("chatlogs/test-id.json", expect.any(String));
