...
export class LogseqChatlogRepository implements IChatlogRepository {
  ...
  async saveChatlog(
    ...
    if (storage) {
      ...
      console.warn(
        "[LogseqChatlogRepository] Storage not available, chatlog data will not be persisted to file",
      );
    }
    ...
    if (!pageName) {
      ...
      await this.logseqApi.createPage(
        pageName,
        {
      ...
      console.log(
        `[LogseqChatlogRepository] Renaming chatlog page from "${pageName}" to "${desiredPageName}"`,
      );
    ...
    try {
      await this.logseqApi.upsertPageProperty(
        pageName,
        CHATLOG_PROPERTIES.UPDATED,
  ...
  async loadChatlog(id: string): Promise<ChatlogEntry | null> {
    ...
    const getProp = (obj: any, key: string) => {
      ...
      return obj[key] || obj[key.replace(/\./g, "-")] || obj[key.replace(/\./g, "_")];
    };
    ...
    const storages = [
      this.logseqApi.getGraphStorage?.(),
      this.logseqApi.getPluginStorage?.(),
    ].filter(Boolean);
    ...
    for (const storage of storages) {
      ...
      try {
        ...
        if (jsonContent) {
          ...
          console.log(
            `[LogseqChatlogRepository] Loaded ${messages.length} messages from JSON file`,
          );
    ...
    const metadata: ChatlogMetadata = {
      ...
      updated:
        getProp(props, CHATLOG_PROPERTIES.UPDATED) ||
        (page.updatedAt ? new Date(page.updatedAt).toISOString() : new Date().toISOString()),
    ...
    try {
      ...
      for (const result of results) {
        ...
        try {
          ...
          const getProp = (obj: any, key: string) => {
            if (!obj) return undefined;
            return obj[key] || obj[key.replace(/\./g, "-")] || obj[key.replace(/\./g, "_")]; // specific Logseq normalization sometimes
          };
