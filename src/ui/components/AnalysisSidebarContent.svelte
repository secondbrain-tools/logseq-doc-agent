...
import { groupByCategory, getSeverity, genericClick } from "./evaluation/evaluation-review-logic";
...
const filteredCategories = $derived(() => {
  ...
  for (const [categoryName, criteria] of Object.entries(groups)) {
    const filteredCriteria = criteria
      .filter((c) => {
        const matchesSearch = c.criterion_id.toLowerCase().includes(searchQuery.toLowerCase());
        const matchesScore = showLowScoresOnly ? c.score <= scoreThreshold && c.score > 0 : true;
    ...
    if (filteredCriteria.length > 0) {
      const sum = filteredCriteria.reduce((acc, c) => acc + c.score, 0);
      const avg =
        filteredCriteria.length > 0 ? Math.round((sum / filteredCriteria.length) * 10) / 10 : 0;
  ...
  <div class="lda-analysis-header">
    ...
    <div class="lda-toolbar">
      <div class="lda-filter-toggles">
        <label class="lda-toggle" title="Show only criteria with low scores">
      ...
      <button
        class="lda-toolbar-btn {showSearch ? 'lda-toolbar-btn--active' : ''}"
        title="Toggle search"
        use:genericClick={() => {
        ...
        <div class="lda-accordion-item">
          ...
          >
            <div class="lda-accordion-title-row">
              ...
              <EvaluationScore rating={category.avgScore} showValue={true} size="sm" />
