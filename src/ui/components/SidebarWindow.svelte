...
let EffectiveHeaderActions = $derived(headerActions || componentProps?.headerActions);
let effectiveHeaderActionsProps = $derived(
  headerActionsProps || componentProps?.headerActionsProps || {},
);
let effectiveMenuOptionsRaw = $derived(menuOptions || componentProps?.menuOptions || []);
...
let effectiveMenuOptions = $derived([
  ...effectiveMenuOptionsRaw,
  {
    ...
    action: isPoppedOut
      ...
      : () => popoutManager.togglePopout(Component, componentProps, isPoppedOut),
  ...
  <div class="flex flex-col w-full relative">
    ...
    >
      <button
        ...
        type="button"
      >
        <span class="opacity-50 hover:opacity-100 flex items-center pr-1">
          <span class="rotating-arrow {isCollapsed ? '' : 'not-collapsed'}">
      ...
      <div class="item-actions flex items-center">
        ...
        {@render headerButton(
          "Close",
          `<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-x" width="18" height="18" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>`,
          (e) => {
            e.stopPropagation();
            closePanel();
          },
    ...
    <div
      role="region"
      class="sidebar-item-content {isCollapsed ? 'hidden' : 'initial'} bg-white lda-sidebar-content"
    >
      <div class="contents flex-col flex">
        {#if isPoppedOut}
          ...
          >
            ...
            <h3 class="text-lg font-medium mb-2">Chat Popped Out</h3>
            <p class="text-sm opacity-70 mb-4">
              The chat window is currently open in a separate window.
            </p>
            <button
