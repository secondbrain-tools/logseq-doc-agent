...
let { agents = [], value = $bindable(), disabled = false, onChange, onOpen }: Props = $props();
...
<div class="lda-agent-selector">
  ...
  {#if isOpen}
    <div bind:this={popoverRef} class="lda-agent-dropdown-portal" style={popoverStyle}>
...
<!-- Full Screen Modal -->
<ChatModal title="Select AI Agent" isOpen={isModalOpen} onClose={() => (isModalOpen = false)}>
  ...
  <div class="lda-agent-list-large">
      ...
      {#each enabledAgents as agent}
        ...
        >
          <div class="lda-agent-card-header">
            ...
            {#if agent.name === value}
              <div class="lda-agent-selected-indicator">Selected</div>
            {/if}
          </div>
    ...
    {#if agents.length === 0}
      <div class="lda-no-agents-large">
        No agents found. Check your Logseq graph for agent configurations.
      </div>
    {/if}
  </div>
</ChatModal>
