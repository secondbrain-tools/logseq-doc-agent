...
let { isOpen, onClose, onNewChat, onLoadChatlog, onDeleteChatlog, onListChatlogs }: Props =
  $props();
...
function groupedChatlogs() {
  ...
  return order.map((key) => ({ title: key, items: groups[key] })).filter((g) => g.items.length > 0);
}
</script>
  ...
  <div class="lda-history-list">
    ...
    {:else if groupedChatlogs().length === 0}
      <div class="lda-history-empty">
        {searchQuery ? "No chatlogs match your search" : "No chat history yet"}
      </div>
    {:else}
          ...
          <div class="lda-history-group-items">
            {#each group.items as chatlog (chatlog.id)}
              <div
                ...
                onkeydown={(event) => handleItemKeydown(event, chatlog.id)}
                role="button"
                tabindex="0"
              ...
              >
                <div class="lda-history-item-content">
                  ...
                  <div class="lda-history-item-meta">
                    <span>{formatDate(chatlog.updated || chatlog.created)}</span>
                  </div>
                </div>
                <button
                  class="lda-history-item-delete {deleteConfirmId === chatlog.id ? 'confirm' : ''}"
                  use:clickHandler={(e) => handleDelete(chatlog.id, e)}
                  title={deleteConfirmId === chatlog.id ? "Click again to confirm" : "Delete"}
                >
