...
import { getCurrentPageContext, onCurrentPageChange } from "../../../infra/logseq/context-utils";
...
let highlightedBubbleIndex = $state<number | null>(null);
let bubbleMaximizeRequest = $state(0);
...
function loadConfiguredModels(settings: any) {
  ...
  for (const provider of PROVIDERS) {
    ...
    const isProviderEnabled =
      settings[providerKey] !== undefined ? settings[providerKey] : defaultProviderEnabled;
    ...
    for (const model of provider.models) {
      ...
      const isModelEnabled =
        settings[modelKey] !== undefined ? settings[modelKey] : model.defaultEnabled;
      ...
      if (isModelEnabled) {
        ...
        const supportsReasoning =
          customReasoningEnabled !== undefined ? customReasoningEnabled : model.supportsReasoning;
  ...
  if (groups.length > 0) {
    ...
    const currentIsValid = allModels.some((m) => m.id === selectedModel);

    // 2. Get default from settings
    const defaultModelSetting = settings["model"];
    const defaultIsValid =
      defaultModelSetting && allModels.some((m) => m.id === defaultModelSetting);
...
async function handleSubmit() {
  ...
  selectedPrompts = [];
  activeContexts = activeContexts.filter((c) => c.isAuto).map((c) => ({ ...c, isActive: false }));
}
...
function updateAutoContext(ctx: ContextItem | null) {
  ...
  const wasActive = existingAutoIndex !== -1 ? activeContexts[existingAutoIndex].isActive : true; // Default to true if new

  let newContexts = activeContexts.filter((c) => !c.isAuto);

  if (ctx) {
    // Prepend new auto context with restored state
    newContexts = [{ item: ctx, isActive: wasActive, isAuto: true }, ...newContexts];
  }
...
function refreshPrompts() {
  Services.instance.promptTemplateService.listPrompts().then((p: ChatPrompt[]) => {
    availablePrompts = p;
  });
}
...
function handleBubbleSelect(index: number) {
  if (highlightedBubbleIndex === index) {
    highlightedBubbleIndex = null;
  } else {
    highlightedBubbleIndex = index;
  }
}
...
function requestMaximizeHighlighted() {
  if (highlightedBubbleIndex !== null) {
    bubbleMaximizeRequest++;
  }
}

function handleKeydown(e: KeyboardEvent) {
  // Only handle when not typing in the input
  const tag = (e.target as HTMLElement)?.tagName?.toLowerCase();
  if (tag === "textarea" || tag === "input") return;

  if (e.key === "m" && !e.ctrlKey && !e.metaKey && !e.altKey) {
    e.preventDefault();
    requestMaximizeHighlighted();
    ...
    if (highlightedBubbleIndex !== null) {
      e.preventDefault();
      // If the modal is open (odd toggle count), close it first.
      // Defer unhighlight via setTimeout so the MessageBubble $effect
      // processes the maximizeSignal change before index is nulled.
      if (bubbleMaximizeRequest % 2 === 1) {
        bubbleMaximizeRequest++;
      }
      setTimeout(() => unhighlightBubble(), 0);
    }
  }
}
function handleContextMenu(e: MouseEvent, msg: Message, msgIndex: number) {
  e.preventDefault();
  e.stopPropagation();
...
async function copyMessageToClipboard(msg: Message) {
  ...
  if (!msg.parts || msg.parts.length === 0) {
    ...
    const parts = msg.parts.filter((p) => ["content", "reasoning"].includes(p.type));
...
$effect(() => {
  ...
  if (msgs.length === 0) {
    untrack(() => {
      activeContexts = activeContexts.map((c) => (c.isAuto ? { ...c, isActive: true } : c));
    });
  }
});
...
let groupedMessages = $derived.by(() => {
  ...
  const isToolMsg = (m: Message) =>
    m.parts && m.parts.some((p) => p.type === "tool_call" || p.type === "tool_result");
  ...
  for (const msg of msgs) {
    ...
    const currentIsMergeTarget = current.role === "assistant" && isToolMsg(current);
    ...
    if (currentIsMergeTarget) {
      if (msg.role === "tool") {
        ...
        if (isToolMsg(msg)) {
          ...
          current.parts = [...currentParts, ...nextParts];
          continue;
        } else if (msg.content && (!msg.parts || msg.parts.length === 0)) {
...
// --- Settings ---
let maximizedWidth = $derived($settingsStore?.maximizedChatWidth || "900px");
...
<div
  class="lda-chat-container"
  style="--lda-max-width: {maximizedWidth}"
  ...
  <div bind:this={messageContainer} class="lda-chat-messages">
    ...
    <div class="lda-messages-inner">
      {#each displayMessages as msg, mIndex (msg.id)}
        <MessageBubble
          {msg}
          onToggleCollapse={(pIndex) => togglePartCollapse(mIndex, pIndex)}
          onContextMenu={(e, m) => handleContextMenu(e, m, mIndex)}
          onSelect={() => handleBubbleSelect(mIndex)}
          isHighlighted={highlightedBubbleIndex === mIndex}
          maximizeSignal={highlightedBubbleIndex === mIndex ? bubbleMaximizeRequest : 0}
        />
      {/each}
        ...
        <div class="flex justify-center p-2 animate-fade-in">
          ...
          >
            <svg
              ...
              ><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M7 7v10l10 -5z" /></svg
            >
            Continue generating
          </button>
        </div>
  ...
  {#if contextMenu.visible && contextMenu.message}
    <ContextMenu
      ...
      options={[
        ...
        // Maximize Response - only for assistant messages with text
        ...(contextMenu.message?.role === "assistant" &&
        (contextMenu.message?.content?.trim() ||
          contextMenu.message?.parts?.some((p) => p.type === "content"))
          ? [
              {
                label: "Maximize Response",
                action: () => {
                  if (contextMenu.msgIndex >= 0) {
                    highlightedBubbleIndex = contextMenu.msgIndex;
                    bubbleMaximizeRequest++;
                  }
                },
              },
...
<style>
  ...
  :global(.lda-sidebar-maximized) .lda-input-wrapper {
    padding-inline: max(1rem, calc((100% - var(--lda-max-width, 900px)) / 2));
  }
  ...
  :global(.lda-sidebar-maximized) .lda-input-wrapper :global(.lda-chat-input-area) {
