...
let { isOpen, title, onClose, children, headerActions, overflowVisible = false }: Props = $props();
...
// Svelte action for keydown (workaround: Svelte 5 event directives may fail in Logseq)
function keydownAction(node: HTMLElement) {
  function handler(e: KeyboardEvent) {
    ...
    if (e.key === "Escape") {
      e.preventDefault();
      e.stopPropagation(); // Don't bubble to chat container
      onClose();
...
{#if isOpen}
  ...
  >
    <div class="lda-chat-modal-container">
      ...
      <!-- Content -->
      <div class="lda-chat-modal-content" style={overflowVisible ? "overflow: visible;" : ""}>
        {@render children()}
      </div>
    </div>
  </div>
