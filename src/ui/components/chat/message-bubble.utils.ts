...
export function findToolResult(
  ...
  return message.parts.find((p) => p.type === "tool_result" && p.toolCallId === toolCallId);
}
...
export function getToolSummary(part: MessagePart, result?: any): string {
  ...
  switch (part.toolName) {
    ...
    case "deleteBlock":
      ...
      return delContent ? `"${truncate(delContent)}"` : args.id ? `#${args.id}` : "";

    case "moveBlock":
      const movContent = extractQuote(output);
      return movContent ? `"${truncate(movContent)}"` : args.id ? `#${args.id}` : "";
    ...
    case "getBlock":
      ...
      return blockContent ? `"${truncate(blockContent)}"` : args.id ? `#${args.id}` : "";
...
export type DisplayItem = { type: "part"; part: MessagePart; index: number } | ToolGroup;
