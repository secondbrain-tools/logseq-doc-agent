...
import type { Issue, BlockEvaluation } from "../../../domain/evaluation/entity";
...
const isDone = $derived(
  issue.status === "resolved" || issue.user_feedback?.some((fb) => fb.type === "done") || false,
);

const isIgnored = $derived(issue.status === "ignored");

const doneFeedbackIdx = $derived(issue.user_feedback?.findIndex((fb) => fb.type === "done") ?? -1);

const hasChangeProposal = $derived(
  issue.user_feedback?.some((fb) => fb.type === "change_proposal") ?? false,
);
...
function toggleSuggestionPreview(idx: number, suggestion: any) {
  ...
  if (activeSuggestionIdx === idx) {
    ...
    // Select and preview
    console.log(`[EvaluationIssueBlock] new index ${idx} selected, applying preview`);
...
async function handleSaveFeedback(type: "reply" | "change_proposal", text: string) {
  if (!text.trim()) return;
  const updated = await saveFeedback(blockId, criterionId, issueIdx, type, text, evaluationData);
...
async function handleDeleteFeedback(fbIdx: number) {
  const updated = await deleteFeedback(blockId, criterionId, issueIdx, fbIdx, evaluationData);
  onDataUpdate(updated);
}
...
async function handleToggleDone(e?: MouseEvent) {
  ...
  if (issue.status === "resolved" || issue.status === "ignored" || isDone) {
    // Reopen
    const updated = await setIssueStatus(blockId, criterionId, issueIdx, "open", evaluationData);
...
async function handleIgnoreIssue(e?: MouseEvent) {
  ...
  const updated = await setIssueStatus(blockId, criterionId, issueIdx, "ignored", evaluationData);
  onDataUpdate(updated);
}
...
async function handleDismissSuggestion(sIdx: number, e?: MouseEvent) {
  ...
  if (activeSuggestionIdx === sIdx) {
    if (blockId) Services.instance.evidenceHighlightService.clearPreview();
    activeSuggestionIdx = null;
  }
}
...
const showPreCommitmentPrompt = $derived(
  needsPreCommitment(issue, preCommitmentEnabled, fallbackPreCommitmentSuggestions, issueUniqueId),
);

const savedPreCommitment = $derived(
  getPreCommitmentSuggestion(issue, fallbackPreCommitmentSuggestions, issueUniqueId),
);
...
function clickOutsideAction(node: HTMLElement) {
  const handleClick = (e: MouseEvent) => {
    ...
    setTimeout(() => {
      ...
      const isInsideInteractable = target.closest?.(".lda-suggestion-item, .lda-suggestion-card");

      if ((isOutsideIssueBlock || !isInsideInteractable) && activeSuggestionIdx !== null) {
        activeSuggestionIdx = null;
        if (blockId) Services.instance.evidenceHighlightService.clearPreview();
      }
    }, 0);
  ...
  <div class="lda-issue-header flex items-center justify-between mb-1">
    <div class="flex items-center gap-2">
      <span
        ...
        style={isDone || isIgnored ? "opacity: 0.8; font-weight: normal;" : "font-weight: 600;"}
      >
        {#if isDone}
          <span
    ...
    <div transition:slide|local>
      {#if showPreCommitmentPrompt}
        ...
        >
          ...
          <div class="text-xs mb-2 opacity-80">
            Before seeing the AI's suggestions, propose your own fix for this issue.
          </div>
          <textarea
          ...
          <div class="flex justify-end gap-2">
            <button type="button" class="lda-action-btn" use:genericClick={skipPreCommitment}
              >Skip</button
            >
            <button
              type="button"
              ...
              use:genericClick={() => handleSavePreCommitment(preCommitmentInputText)}
      ...
      {:else}
        {#if savedPreCommitment && savedPreCommitment !== "__skipped__"}
          ...
          >
            <span class="font-semibold" style="color: var(--ls-link-text-color);"
        ...
        {#if issue.suggestions && issue.suggestions.length > 0}
          <div class="lda-issue-suggestions text-xs">
              ...
              <div class="lda-alternatives-container mt-1">
                {#each issue.suggestions as suggestion, i}
                  {#if suggestion.status === "dismissed"}
                    ...
                    >
                      <span class="italic text-gray-500 line-through"
                        >Option {String.fromCharCode(65 + i)} dismissed</span
                      >
                      <button
                        type="button"
                        class="lda-action-btn-icon hover:text-blue-600"
                        title="Reactivate"
                        use:genericClick={(e) => handleReactivateSuggestion(i, e)}
                      >
                        {@html ICONS.undo}
                      </button>
                    </div>
                  {:else}
                    ...
                    <div
                      class="lda-suggestion-card {activeSuggestionIdx === i
                      ...
                      use:genericClick={() => {
                        if (suggestion.status !== "accepted")
                          toggleSuggestionPreview(i, suggestion);
                      }}
                    >
                        ...
                        {#if suggestion.proposed_text}
                          <span
                            ...
                            'accepted'
                              ...
                              : ''} dark:bg-blue-900 px-1 rounded">{suggestion.proposed_text}</span
                          >
                        {/if}
                        <div
                        ...
                        {#if activeSuggestionIdx === i && suggestion.status !== "accepted"}
                          ...
                          >
                            <button
                              ...
                              title="Dismiss"
                              use:genericClick={(e) => handleDismissSuggestion(i, e)}
                            >
                              <span class="scale-125 flex">{@html ICONS.close}</span>
                            </button>
                            <button
                              type="button"
                              ...
                              title="Apply"
                              use:genericClick={(e) => handleApplySuggestion(i, e)}
                            >
                              <span class="scale-125 flex">{@html ICONS.check}</span>
                            </button>
                          </div>
                        {/if}
                      </div>
            ...
            {:else}
              <ul class="lda-suggestions-list list-disc pl-4 mt-1">
                {#each issue.suggestions as suggestion, i}
                  {#if suggestion.status === "dismissed"}
                    <li
                      class="lda-suggestion-item lda-suggestion-dismissed text-gray-500 italic pb-1 flex justify-between"
                    >
                      <span class="line-through flex-1">Suggestion dismissed</span>
                      <button
                        type="button"
                        class="lda-action-btn-icon hover:text-blue-600"
                        title="Reactivate"
                  ...
                  {:else}
                    ...
                    <li
                      ...
                      'accepted'
                        ...
                        : 'cursor-pointer'} {activeSuggestionIdx === i
                      ...
                      {#if activeSuggestionIdx === i && suggestion.status !== "accepted"}
                        ...
                        >
                          <button
                            ...
                            use:genericClick={(e) => handleApplySuggestion(i, e)}
                          >
                            <span class="scale-100 flex">{@html ICONS.check}</span>
                          </button>
                          <button
                            type="button"
                            class="lda-action-btn flex items-center justify-center w-6 h-6 rounded shrink-0 border border-red-200 text-red-600 hover:bg-red-50 p-0"
                            ...
                            use:genericClick={(e) => handleDismissSuggestion(i, e)}
                          >
                            <span class="scale-100 flex">{@html ICONS.close}</span>
                          </button>
                        </div>
                      {/if}
                    </li>
        ...
        {#if issue.counterargument}
          ...
          >
            <div class="lda-counterargument-header flex items-center gap-2 mb-1">
              <span class="font-semibold text-xs text-gray-700 dark:text-gray-300"
                >↯ Counterargument</span
              >
            </div>
            <div class="lda-counterargument-text text-xs italic opacity-90">
          ...
          <div class="lda-user-feedback mt-2 pt-2 border-t">
            <div class="mt-1 flex flex-col gap-[6px]">
              {#each issue.user_feedback
                ...
                <div
                  class="lda-feedback-item bg-gray-50 p-2 rounded text-xs border {item.fb.type ===
                  'ai_reply'
                ...
                >
                  <div class="flex justify-between mb-[2px]">
                    <span class="font-semibold text-gray-600 capitalize opacity-80">
                      {item.fb.type === "ai_reply" ? "🤖 AI Reply" : item.fb.type.replace("_", " ")}
                    </span>
                    <div class="flex gap-2 items-center">
                      <span class="opacity-50 text-[10px]"
                        >{new Date(item.fb.created_at).toLocaleDateString()}</span
                      >
                      {#if item.fb.type === "change_proposal"}
                        <button
                          ...
                          title="Edit proposal"
                          use:genericClick={() => startEditFeedback(item.i, item.fb.text)}>✎</button
                        >
                        <button
                          type="button"
                          class="lda-action-btn-icon opacity-50 hover:opacity-100 text-blue-500"
                          title="Discuss with AI"
                          use:genericClick={handleDiscussProposal}>💬</button
                        >
                      {/if}
                      <button
                        type="button"
                        ...
                        use:genericClick={() => handleDeleteFeedback(item.i)}>×</button
                      >
                    </div>
                  </div>
                    ...
                    <div class="mt-2 text-gray-800">
                      ...
                      <div class="flex justify-end gap-2">
                        <button
                          ...
                          use:genericClick={cancelEditFeedback}>Cancel</button
                        >
                        <button
                          type="button"
                          class="lda-action-btn border-blue-500 text-blue-600 bg-blue-50"
                          use:genericClick={() => handleSaveEditedFeedback(item.i)}>Save</button
                        >
                      </div>
                    </div>
                  {:else}
        ...
        {#if activeFeedbackInput}
          <div class="lda-feedback-input-container mt-2 pt-2 border-t" transition:slide|local>
            <label
              for={feedbackInputId}
            ...
            <div class="flex justify-between items-center mt-1">
              <div class="flex gap-3 items-center text-gray-500" style="font-size: 11px;">
                <label
                ...
                >
                  <input
                    type="checkbox"
                    bind:checked={contextScope.includeDocument}
                ...
                >
                  <input
                    type="checkbox"
                    bind:checked={contextScope.includeEvaluation}
                    style="transform: scale(0.9); margin: 0;"
                  /> Evaluation
                </label>
              </div>
              <div class="flex gap-2">
                <button
                  ...
                  use:genericClick={cancelFeedbackInput}>Cancel</button
                >
                <button
                  type="button"
                  class="lda-action-btn lda-action-btn-sm border-blue-500 text-blue-600 bg-blue-50"
                  use:genericClick={() =>
                    handleSaveFeedback(activeFeedbackInput!, feedbackInputText)}
                  >{activeFeedbackInput === "reply" ? "Send to AI" : "Save Proposal"}</button
                >
              </div>
            </div>
          </div>
          ...
          <div class="lda-feedback-actions mt-2 flex gap-[6px]">
            ...
            {#if !hasChangeProposal}
              <button
                type="button"
                class="lda-action-btn"
                use:genericClick={() => openFeedbackInput("change_proposal")}
              >
                ✎ Propose Change
              </button>
            {/if}
