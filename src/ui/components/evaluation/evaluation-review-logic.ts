...
export async function deleteFeedback(
  ...
  return Services.instance.evaluationReviewService
...
export function startAiIssueReply(
  ...
  if (!blockId) return Promise.resolve(evaluationData);
  return Services.instance.issueReplyService.replyToIssue(
    blockId,
    criterionId,
    issueIdx,
    evaluationData,
  ...
  for (const res of cloned.results) {
    if (res.criterion_id === criterionId && res.issues?.[issueIdx]) {
      ...
      if (blockId) {
        ...
        Services.instance.logseqApi
          .upsertBlockProperty(blockId, "logseq-doc-agent.evaluation", JSON.stringify(cloned))
...
export async function setSuggestionStatus(
  ...
  for (const res of cloned.results) {
    if (
      res.criterion_id === criterionId &&
      res.issues?.[issueIdx] &&
      res.issues[issueIdx].suggestions?.[suggestionIdx]
    ) {
 ...
 */
async function findBlockContainingText(
  startUuid: string,
  exactText: string,
): Promise<string | null> {
...
export async function applySuggestion(
  ...
  if (!targetSuggestion || !targetSuggestion.proposed_text || !targetSelector?.exact) {
    console.error(
      "Suggestion cannot be applied automatically (missing proposed_text or selector.exact).",
      targetSuggestion,
    );
    return null;
  }
  ...
  console.log(
    `[applySuggestion] parentBlockId="${blockId}" rawSourceId="${rawSourceId}" => resolvedBlockId="${resolvedBlockId}"`,
  );
  ...
  try {
    ...
    if (!targetBlockUuid) {
      console.warn(
        `[applySuggestion] Target text not found in block "${resolvedBlockId}" or any of its children. exact="${targetSelector.exact}"`,
      );
      return null;
    }
    ...
    console.log(
      `[applySuggestion] blockText (${blockText.length} chars): "${blockText.substring(0, 120)}${blockText.length > 120 ? "..." : ""}"`,
    );
    ...
    if (!currentBody.includes(targetSelector.exact)) {
      ...
      console.warn(
        `[applySuggestion] Unexpected: text vanished from block "${targetBlockUuid}" between lookup and apply.`,
      );
    } else {
      ...
      await Services.instance.logseqApi.upsertBlockProperty(
        targetBlockUuid,
        LDA_MERGE_PROPERTY,
    ...
    let updatedData = await setSuggestionStatus(
      blockId,
      criterionId,
      issueIdx,
      suggestionIdx,
...
export function getPreCommitmentSuggestion(
  issue: Issue | undefined,
  fallbackSuggestions: Record<string, string>,
  issueUniqueId: string,
): string | null {
...
export function needsPreCommitment(
  issue: Issue | undefined,
