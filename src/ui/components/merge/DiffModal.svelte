    ...
    if (mergeTree && mergeTree.length > 0) {
      ...
      if (activeTree !== mergeTree) {
        ...
        for (const item of activeTree) {
          ...
          if (item.mergeData) {
            edits[item.uuid] = item.mergeData.currentContent || item.content || "";
          } else {
...
function handleToggle(uuid: string, recursive: boolean = false) {
  console.log("[DiffModal] handleToggle called for:", uuid, "recursive:", recursive);
...
function handleReplaceRequest(uuid: string, source: "original" | "new", subtree: boolean) {
  console.log(`[DiffModal] Replace Request: uuid=${uuid}, source=${source}, subtree=${subtree}`);
  ...
  <div class="lda-diff-container">
    ...
    <div class="lda-diff-editor-wrapper">
      {#if isOpen}
        {#if activeTree.length > 0}
          <!-- Tree Edit Mode -->
          <div class="tree-diff-container" class:centered={!showPreview}>
            <!-- Global Headers -->
        ...
        {:else if mergeData}
          <!-- Single Block Edit -->
          <div class="single-edit-layout" class:centered={!showPreview}>
            ...
            {#if showPreview}
              <div class="single-edit-col">
                <PreviewPane content={editContent} originalContent={mergeData.base || ""} />
              </div>
            {/if}
          </div>
        {/if}
    ...
    <div class="lda-diff-actions">
      <button class="lda-btn lda-btn-secondary" use:genericClick={handleClose} type="button"
        >Cancel</button
      >
      <button class="lda-btn lda-btn-primary" use:genericClick={handleAccept} type="button"
        >Save Result</button
      >
    </div>
  </div>
