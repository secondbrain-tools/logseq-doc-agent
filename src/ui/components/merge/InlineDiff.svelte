...
function calculateDiff() {
  ...
  const result = calculateDiffLines(originalContent, modifiedContent, mode, currentDecisions);
...
function handleSelectionAction(action: "accept" | "revert") {
  ...
  const findPartType = (id: string): "added" | "removed" | "replacement" | "common" | null => {
...
function handleBlockLineClick(e: MouseEvent, lineId: string, blockId: string, role: "new" | "old") {
  const selection = window.getSelection();
...
function getDecisionClass(partId: string | undefined, type: string): string {
  if (!partId) return "";
...
function unifiedPartClickAction(node: HTMLElement, fn: (e: MouseEvent) => void) {
  const handler = (e: MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    fn(e);
      ...
      <div class="diff-line type-{line.type}">
        <div class="gutter">
          {#if i === 0 && canToggle}
            ...
            <!-- svelte-ignore a11y_no_static_element_interactions -->
            <div class="toggle-btn" use:manualClick={onToggle} role="button" tabindex="0">
              {isExpanded ? "▼" : "▶"}
            </div>
          {/if}
        ...
        <div class="line-content">
          <span class="marker">
            {line.type === "added" || line.type === "modified-new"
              ? "+"
              : line.type === "removed" || line.type === "modified-old"
                ? "-"
          ...
          <span class="text">
            ...
            {:else if line.intraLineParts && line.intraLineParts.length > 0}
              {#each line.intraLineParts as part}
                <span class="intra-{part.type}">{part.text}</span>
              {/each}
            {:else if line.blockRole && line.id && line.blockId}
              ...
              <span
                ...
                use:unifiedPartClickAction={(e) =>
                  handleBlockLineClick(e, line.id!, line.blockId!, line.blockRole!)}
              >
                {line.content}
              </span>
            {:else if line.id}
