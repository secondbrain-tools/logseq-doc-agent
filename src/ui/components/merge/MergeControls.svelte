...
const mergeTreeService = $derived(mode === "block" ? new MergeTreeService() : (null as any));
const mergeActionService = $derived(mode === "block" ? new MergeActionService() : (null as any));
...
function toggleReveal(shouldReveal: boolean) {
  ...
  if (blockContainer) {
    blockContainer.classList.toggle("lda-reveal-children", shouldReveal);
  }
}
...
function handleActionHover(action: "accept" | "revert" | "diff", isEnter: boolean, e?: MouseEvent) {
  if (!targetElement) return;
  const blockContainer = targetElement.closest(".ls-block");
  if (!blockContainer) return;
...
function setupGlobalListeners() {
  ...
  if (parent && parent !== window) {
    try {
      parent.document.addEventListener("keyup", handleKeyChange, true);
      parent.document.addEventListener("keydown", handleGlobalKeydown, true);
    } catch (e) {}
  }
}
...
function removeGlobalListeners() {
  ...
  if (parent && parent !== window) {
    try {
      parent.document.removeEventListener("keyup", handleKeyChange, true);
      parent.document.removeEventListener("keydown", handleGlobalKeydown, true);
    } catch (e) {}
  }
}
...
onMount(() => {
  ...
  const blockDiv = parent.document.querySelector(`div[blockid="${blockUuid}"]`);
  if (blockDiv) {
    ...
    if (targetElement) {
      ...
      if (mergeData && (mergeData.type === "add" || mergeData.type === "delete")) {
        targetElement.classList.add(`lda-merge-type-${mergeData.type}`);
      }
    }
  }
  ...
  if (blockDivForFocus) {
    blockDivForFocus.addEventListener("lda-focus-merge-controls", handleFocusCommand);
  }
...
onDestroy(() => {
  const blockDivForFocus = targetElement?.closest("div[blockid]");
  if (blockDivForFocus) {
    blockDivForFocus.removeEventListener("lda-focus-merge-controls", handleFocusCommand);
  }
...
function focusControls() {
  if (controlsDiv) {
    const firstBtn = controlsDiv.querySelector(".lda-merge-accept") as HTMLElement;
...
async function handleAccept(
  ...
  try {
    ...
    if (event && event.detail && event.detail.treeEdits) {
      await mergeActionService.acceptBatchMerge(event.detail.treeEdits, patterns);
    } else if (event && event.detail && typeof event.detail.content === "string") {
      await mergeActionService.acceptMerge(blockUuid as string, event.detail.content, patterns);
    }
...
async function handleQuickAccept(e: MouseEvent) {
  ...
  try {
    ...
    if (withChildren) {
      await mergeActionService.quickAcceptWithChildren(blockUuid as string);
      await logseq.UI.showMsg("Accepted block and children", "success");
    } else {
...
async function handleRevert(e?: MouseEvent) {
  ...
  try {
    ...
    if (withChildren) {
      await mergeActionService.revertMergeWithChildren(blockUuid as string);
      await logseq.UI.showMsg("Reverted block and children", "success");
    } else {
      const uuids = mergeTree.length > 0 ? mergeTree.map((i) => i.uuid) : [blockUuid as string];
...
async function handleDiff(e?: MouseEvent) {
  ...
  try {
    const patterns = getMergeFilterPatterns();
    const tree = await mergeTreeService.getMergeTree(blockUuid as string);

    const processedTree: MergeTreeItem[] = [];

    for (const item of tree) {
      const [cleanContent] = filterProperties(item.content, patterns);
      const mergeDataClone = item.mergeData ? { ...item.mergeData } : undefined;
      ...
      if (mergeDataClone) {
        ...
        if (mergeDataClone.base) {
          const [cleanBase] = filterProperties(mergeDataClone.base, patterns);
...
{#snippet ActionButtons(isIndicator: boolean)}
  <button
    ...
    onmouseenter={isIndicator ? null : (e) => handleActionHover("accept", true, e)}
    onmouseleave={isIndicator ? null : () => handleActionHover("accept", false)}
    onfocus={isIndicator ? null : (e) => handleActionHover("accept", true, e as any)}
  ...
  {#if mode === "block" || isIndicator}
    <button
      ...
      onmouseenter={isIndicator ? null : (e) => handleActionHover("diff", true, e)}
      onmouseleave={isIndicator ? null : () => handleActionHover("diff", false)}
      onfocus={isIndicator ? null : (e) => handleActionHover("diff", true, e as any)}
      onblur={isIndicator ? null : () => handleActionHover("diff", false)}
      title={isIndicator ? "Diff (Preview)" : "Show Diff for this block and its children."}
    >
      <svg
        viewBox="0 0 24 24"
        fill="none"
  ...
  <button
    ...
    onmouseenter={isIndicator ? null : (e) => handleActionHover("revert", true, e)}
    onmouseleave={isIndicator ? null : () => handleActionHover("revert", false)}
    onfocus={isIndicator ? null : (e) => handleActionHover("revert", true, e as any)}
...
<div
  ...
  style={mode === "selection" ? "display: inline-flex; opacity: 1; transform: none;" : ""}
  role="group"
