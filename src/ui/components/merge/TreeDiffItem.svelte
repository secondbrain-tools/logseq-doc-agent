...
let {
  ...
  onReplaceRequest?: (uuid: string, source: "original" | "new", subtree: boolean) => void;
} = $props();
...
$effect(() => {
  if (initializedItemUuid !== item.uuid) {
    ...
    editContent = currentContent ?? item.mergeData?.currentContent ?? item.content;
    return;
  }
  ...
  if (currentContent !== undefined && currentContent !== lastKnownCurrentContent) {
    lastKnownCurrentContent = currentContent;
...
<div class="tree-diff-item" style="margin-left: {item.level * 20}px" data-block-uuid={item.uuid}>
  <div class="diff-block">
    ...
    >
      ...
      <div class="smart-row">
        <div class="smart-col smart-input {showPreview ? 'input-with-tools' : ''}">
          <!-- Word-level Unified Diff (interactive accept/revert) -->
          <div class="diff-wrapper">
            <InlineDiff
              originalContent={item.mergeData?.base ?? item.content}
              modifiedContent={stableUnifiedContent}
              canToggle={true}
              {isExpanded}
              onToggle={(recursive) => handleInteraction(recursive)}
              mode="words"
