...
export type DiffLine = {
  content: string;
  type: "common" | "added" | "removed" | "modified-old" | "modified-new" | "modified-unified";
...
export function computeUnifiedParts(wordDiff: Diff.Change[], lineIndex: number): UnifiedPart[] {
  ...
  while (i < wordDiff.length) {
    ...
    if (current.removed && i + 1 < wordDiff.length && wordDiff[i + 1].added) {
...
export function calculateDiffLines(
  ...
  if (!originalContent && !modifiedContent)
    return { diffLines: [], newDecisions: initialDecisions };
  ...
  changes.forEach((part) => {
    ...
    const type: RawLine["type"] = part.added ? "added" : part.removed ? "removed" : "common";
      ...
      if (removedLines.length > 0 && addedLines.length > 0) {
        ...
        for (let k = 0; k < maxPairs; k++) {
          ...
          if (sim >= SIMILARITY_THRESHOLD && mode === "words") {
            ...
            const parts = computeIntraLineParts(oldLine, newLine);
            const showIntraLine = shouldShowIntraLine(oldLine, newLine, parts);

            if (mode === "words") {
              // Unified Word Mode
              const wordDiff = Diff.diffWordsWithSpace(oldLine, newLine);
              const unifiedParts = computeUnifiedParts(wordDiff, oldCounter);

              for (const part of unifiedParts) {
                if (part.id && part.type !== "common" && !newDecisions[part.id]) {
    ...
    if (line.type === "common") {
      ...
      for (const part of line.unifiedParts) {
        ...
        if (part.type === "common") {
          ...
          if (decision === "accept") lineText += part.addedText || "";
