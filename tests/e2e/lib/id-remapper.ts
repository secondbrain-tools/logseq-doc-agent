 ...
 */
function buildContentIndex(blocks: SnapshotBlock[], index: Map<string, number> = new Map()): Map<string, number> {
 ...
 */
export function buildIdRemap(sourceTree: SnapshotPage, targetTree: SnapshotPage): Map<number, number> {
            ...
            if (key.length > 0 && targetIndex.has(key)) {
                ...
                for (const [targetKey, targetId] of targetIndex) {
                    if (targetKey.includes(key) || key.includes(targetKey)) {
                        console.log(`[IdRemapper] Fuzzy match: source ${block.id} "${key}" ~ target ${targetId} "${targetKey}"`);
