...
export const logseq = {
  ...
  Editor: {
    ...
    getBlock: async (uuid, opts) => {
      ...
      if (
        !block &&
        (typeof uuid === "number" || (typeof uuid === "string" && /^\d+$/.test(uuid)))
      ) {
      ...
      if (block) {
        ...
        if (!propName.startsWith("logseq-doc-agent")) {
          const normalize = (k) =>
            k
              .split(".")
              .map((part) => part.replace(/-./g, (x) => x[1].toUpperCase()))
              .join(".");
...
logseq.Editor.getPage = async (name) => {
  ...
  if (page) {
    return {
      ...
      uuid:
        "mock-page-uuid-" +
        (page.properties["lda.chatlog.id"] || Math.random().toString(36).substr(2, 5)),
    };
  }
  return null;
};
...
logseq.Editor.getPageBlocksTree = async (name) => {
  ...
  const isCurrentPage =
    !name ||
    name === currentTitle ||
    name === "Embracing Imperfection Guide" ||
    name === "page-uuid-123" ||
    name === "start-page";
  ...
  const page = logseq._pages.find(
    (p) => p.name === name || p.originalName === name || p.uuid === name,
  );
  if (page && page.blocks) {
  ...
  console.warn(
    `[MockLogseq] getPageBlocksTree: Page "${name}" not found. Returning attributes of current page.`,
  );
  const blocks = getBlocks();
  return blocks.map(mapNode);
};
...
logseq.Editor.appendBlockInPage = async (pageId, content) => {
  ...
  const page = logseq._pages.find(
    (p) => p.name === pageId || p.originalName === pageId || p.uuid === pageId,
  );
  if (page) {
    const newBlock = {
          ...
          children: [
            {
              ...
              content:
                "You are a helpful AI assistant for Logseq. Help users research, write, and manage notes.",
              properties: {},
              children: [],
            },
          ],
          ...
          children: [
            {
              ...
              content:
                "You are a research assistant. You can read documents but cannot modify them. Focus on analysis and summarization.",
              properties: {},
              children: [],
            },
          ],
          ...
          children: [
            {
              ...
              content:
                "You are a writing assistant. Help users draft and add content to their notes. You can read documents and add new blocks.",
              properties: {},
              children: [],
            },
          ],
